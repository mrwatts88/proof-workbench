...
| C034 | computational observation | E015 bipartite hunt (CPython 3.14.2 and PyPy 7.3.23; nauty 2.9.3 `genbg -q -c -Z1 -d2 p q mine:0`, \(\text{mine}=\lceil(3n-2)/2\rceil\), splits pruned by `L035` T5): **no connected bipartite \(C_4\)-free graph with at most two sub-cubic vertices (each of degree \(\ge2\)) is power-free at any order \(n\le21\)** — hence, with the pendant reduction, no witness of `L035` T3 of order \(\le22\) with a degree-1 vertex either. Class sizes / minimum \(C_8\) count by order: 12–13: 0; 14: 2 (min 13 \(C_8\)s); 15: 1 (21); 16: 6 (20); 17: 8 (27); 18: 75 (23); 19: 197 (26); 20: 2,715 (18); 21: 10,865 (24). An order-22 run was launched in S016 and was still inside its \((10,12)\) split at session close on a heavily loaded machine; **order 22 is deliberately not part of this row** and its harvest is a follow-up leg. **Every member of the class at every scanned order contains a \(C_8\)** — the \(C_{16}\) test is never decisive, and the minimum \(C_8\) count never drops below 13, so the class is far from a witness (contrast the pinched channel's equality blocks, blocked by 3–7 \(C_8\)s). Sub-results at the same orders: no bipartite 1-atom (the one-sub-cubic-vertex column: 332 candidates at order 20, 1,208 at 21, all with a \(C_8\)); no bipartite counterexample to 0.1 (the min-degree-\(\ge3\) column: 50 at order 20, 69 at 21), which verifies internally the sub-range of the unverified `G014` item-2 import. The order-14 class is exactly the Heawood graph (cubic, 21 \(C_8\)s, identified by `labelg`) and one 20-edge graph with terminals in opposite classes (all-odd \(S\), 13 \(C_8\)s). | tested | `E015` (`anchors` 20,082 checks under both interpreters; `crosscheck 14` proves the genbg-generated class equals the `E010`-stream-derived class by `labelg` set equality; T5 skips validated against genbg at orders 8–14; cycle detector cross-validated against an independent brute-force enumerator on all 994 connected graphs of order \(\le7\) and 1,000+ bipartite graphs of orders 8–11) | genbg/geng/labelg from nauty 2.9.3; `L035` T3/T4/T5 supply the criterion, the completeness reading, and the split pruning; the run says nothing about `L034` channel (ii) |
...
- `C034` is a finite exclusion for one channel, not evidence for 0.1 in
  general: it excludes bipartite witnesses at orders \(\le21\)
  (\(\le22\) with a pendant), where the general stream stops at 15
